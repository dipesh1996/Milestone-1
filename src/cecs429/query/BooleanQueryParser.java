...
public class BooleanQueryParser {
    ...
    /**
     * Locates the start index and length of the next subquery in the given
    ...
    private Literal findNextLiteral(String subquery, int startIndex) {
        ...
        if (subquery.charAt(startIndex) == '-') {
            ...
            return new Literal(
                    new StringBounds(startIndex + 1, lit.bounds.length),
                    new NotQuery(lit.literalComponent));

        }
        if (subquery.charAt(startIndex) == '[') {
            int closebracket = subquery.indexOf(']', startIndex + 1);
            if (closebracket < 0) {
                lengthOut = subLength - startIndex;
            } else {
                lengthOut = closebracket - startIndex + 1;
            }

            return new Literal(
                    new StringBounds(startIndex, lengthOut),
                    new NearLiteral(subquery.substring(startIndex + 1, startIndex + lengthOut - 1)));
